...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { address: @build.address, age: @build.age, name: @build.name, rent: @build.rent } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { address: @build.address, age: @build.age, name: @build.name, rent: @build.rent } }
